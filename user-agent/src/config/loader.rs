...
pub fn load_master_config(path: &Path) -> Result<MasterConfig, Box<dyn std::error::Error>> {
    ...
    let cfg  = toml::from_str::<MasterConfig>(&text)?;
    log::info!("Loaded config from {:?}", path);
    Ok(cfg)
}

/// Convert one `[scanner.*]` block into a runtime group.
pub fn convert_config_to_risk_group(
    risk: DirectoryRisk,
    cfg:  RiskGroupConfig,
) -> RiskGroup {
    let dirs: Vec<PathBuf> = cfg.directories.into_iter().map(PathBuf::from).collect();
    let interval = cfg.scheduled_interval.map(Duration::from_secs);

    log::debug!(
        "Converted {:?} group: {} dirs, interval={:?}",
        risk, dirs.len(), interval
    );

    RiskGroup { risk, directories: dirs, scheduled_interval: interval }
}
